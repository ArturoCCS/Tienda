...
import com.example.dao.IOXUtility;
...
import static com.example.dao.IOXUtility.saveData;
...
public class JFXDecorator extends VBox {
    ...
    private static Map<String, Operable<? extends Keyable>> catalogs;
     ...
     */
    public JFXDecorator(Stage stage, Node node, Map<String, Operable<? extends Keyable>> catalogs) {
        this(stage, node, true, true, true, catalogs);
    }
     ...
     */
    public JFXDecorator(Stage stage, Node node, boolean fullScreen, boolean max, boolean min, Map<String, Operable<? extends Keyable>> catalogs) {
        primaryStage = stage;
        ...
        JFXDecorator.catalogs = catalogs;
        initializeButtons();
            ...
            if (newVal) {
                ...
                windowDecoratorAnimation = new Timeline(new KeyFrame(Duration.millis(320),
                        new KeyValue(this.translateYProperty(),
                                ...
                                Interpolator.EASE_BOTH)));
                windowDecoratorAnimation.setOnFinished((finish) -> {
                    contentPlaceHolder.setBorder(new Border(new BorderStroke(Color.TRANSPARENT,
    ...
    private void initializeButtons() {
        ...
        FontAwesomeIconView iconView = new FontAwesomeIconView(FontAwesomeIcon.EXPAND);
        iconView.setGlyphSize(14);
        btnFull.setGraphic(iconView);
        ...
        btnClose.setOnAction((action) -> {
            try {
                IOXUtility.saveData(catalogs);
                message("Alerta", "Datos guardados exitosamente");
            } catch (Exception err) {
                message("Error", "Error al guardar: " + err.getMessage());
            }
            onCloseButtonAction.get().run();
        });

        FontAwesomeIconView close = new FontAwesomeIconView(FontAwesomeIcon.CLOSE);
        close.setGlyphSize(14);
        ...
        FontAwesomeIconView resizeMin = new FontAwesomeIconView(FontAwesomeIcon.WINDOW_RESTORE);
        resizeMin.setGlyphSize(14);
        FontAwesomeIconView resizeMax = new FontAwesomeIconView(FontAwesomeIcon.WINDOW_MAXIMIZE);
        resizeMax.setGlyphSize(14);
        btnMax.setOnAction((action) -> maximize(resizeMin, resizeMax));
        btnMax.setGraphic(resizeMax);
    }

    private void maximize(FontAwesomeIconView resizeMin, FontAwesomeIconView resizeMax) {
        if (!isCustomMaximize()) {
    ...
    private void initializeContainers(Node node, boolean fullScreen, boolean max, boolean min) {
        ...
        buttonsContainer.setBackground(new Background(new BackgroundFill(Color.TRANSPARENT,
                ...
                Insets.EMPTY)));
        // BINDING
        buttonsContainer.setPadding(new Insets(5, 0, 7, 5));
        ...
        HBox.setMargin(graphicContainer, new Insets(0, 8, 0, -6));
